...
our @ISA = qw(Exporter);
our @EXPORT =
  qw(figureURL yesOrNo deSelectThing startBrowser success isOnPage replaceTextInMultiple clickOnThing replaceTextInThing someText selectThing $FAIL_WAIT);
our ($FAIL_WAIT);
...
BEGIN {
	$ENV{'PUB_DEV'}='https://public-ui.dev.asl.homeoffice.gov.uk/';
	$ENV{'INT_DEV'}='https://internal-ui.dev.asl.homeoffice.gov.uk/';
	$ENV{'PUB_PRE'}='https://public-ui.preprod.asl.homeoffice.gov.uk/';
	$ENV{'INT_PRE'}='https://internal-ui.preprod.asl.homeoffice.gov.uk/';
}

sub figureURL
{

	my $int_pub = shift || die "No int or pub";
	my $url = shift || die "No url";
	if (($int_pub =~ /pub/) && ($url =~ /dev/)) { return $ENV{'PUB_DEV'};}
	if (($int_pub =~ /pub/) && ($url =~ /pre/)) { return $ENV{'PUB_PRE'};}
	if (($int_pub =~ /int/) && ($url =~ /dev/)) { return $ENV{'INT_DEV'};}
	if (($int_pub =~ /int/) && ($url =~ /pub/)) { return $ENV{'INT_PRE'};}
	die "No url";
}
